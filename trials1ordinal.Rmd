...
#  Note, when 'knit' we must be more consistent about the working directory, which now is the /code/ folder, so we can't set the wd to the one shown below, unless, you change the ticked choice in the Menus; Knit> Knit Directory [click triangle] > current working directory
setwd("C:/data/AsianBaro")

#Instead, one has to specify the right folder in each command.
#It is a weakness of using RMD with Knit but is absent when using RMD alone or using R scripts.
```
...
#and it's also critical to type in the filename extension .dta.
write_dta(edusimt,  "C:/data/AsianBaro/data/edtmp.dta" )
```
